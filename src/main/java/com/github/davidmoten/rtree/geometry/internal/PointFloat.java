...
public final class PointFloat implements Point {
	...
	public Rectangle add(Rectangle r) {
		if (r.isDoublePrecision()) {
			return RectangleDouble.create(Math.min(x, r.x1()), Math.min(y, r.y1()), Math.max(x, r.x2()),
					Math.max(y, r.y2()));
			...
			return RectangleFloat.create(Math.min(x, p.x), Math.min(y, p.y), Math.max(x, p.x), Math.max(y, p.y));
		} else {
			PointDouble p = (PointDouble) r;
			return RectangleDouble.create(Math.min(x, p.x()), Math.min(y, p.y()), Math.max(x, p.x()),
					Math.max(y, p.y()));
		}
	}
