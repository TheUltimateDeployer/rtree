...
public final class RectangleFloat implements Rectangle {
	...
	public Rectangle add(Rectangle r) {
		if (r.isDoublePrecision()) {
			return RectangleDouble.create(min(x1, r.x1()), min(y1, r.y1()), max(x2, r.x2()), max(y2, r.y2()));
		} else if (r instanceof RectangleFloat) {
			RectangleFloat rf = (RectangleFloat) r;
			return RectangleFloat.create(min(x1, rf.x1), min(y1, rf.y1), max(x2, rf.x2), max(y2, rf.y2));
		} else {
			PointFloat rf = (PointFloat) r;
			return RectangleFloat.create(min(x1, rf.xFloat()), min(y1, rf.yFloat()), max(x2, rf.xFloat()),
					max(y2, rf.yFloat()));
		}
	}
	...
	public double intersectionArea(Rectangle r) {
		if (!intersects(r))
			...
			return RectangleDouble.create(max(x1, r.x1()), max(y1, r.y1()), min(x2, r.x2()), min(y2, r.y2())).area();
	}
